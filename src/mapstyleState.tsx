...
export const MapStyleState = () => {
	const { mapColor, currentPath } = useMapContext();
	...
	const mapsStyle = {
		...
		sources: {
			...
			vector_mountain: {
				type: "vector",
				url: `https://api.mapbox.com/v4/jvanrees.6lps1j6n/{z}/{x}/{y}.vector.pbf?access_token=${MAPBOX_TOKEN}`,
			},
			historic_buildings: {
				type: "vector",
				url: `https://api.mapbox.com/v4/jvanrees.dm8bkhvt/{z}/{x}/{y}.vector.pbf?access_token=${MAPBOX_TOKEN}`,
			},
			historic_bldg_label_roads: {
				type: "vector",
				url: `https://api.mapbox.com/v4/jvanrees.9n9jyp66/{z}/{x}/{y}.vector.pbf?access_token=${MAPBOX_TOKEN}`,
			},
		...
		layers: [
			{
				...
				paint: {
					...
					"background-opacity":
						currentPath === "/nightmode" ? "visible" : "none",
				},
			},
			{
				id: "night_park",
				...
				paint: {
					...
					"fill-opacity": currentPath === "/nightmode" ? "visible" : "none",
				},
			},
			{
				id: "qing-bg_add",
			...
			},
			{
				...
				paint: {
					...
					"line-opacity": currentPath === "/nightmode" ? "visible" : "none",
				},
			},
			{
				...
				paint: {
					"raster-opacity": currentPath === "/nightmode" ? "visible" : "none",
				},
			},
			{
				id: "night_water",
			...
			},
			{
				...
				paint: {
					"icon-opacity": currentPath === "/nightmode" ? "visible" : "none",
				},
				layout: {
					visibility: currentPath === "/qing-dynasty-map" ? "visible" : "none",
					"icon-size": 0.5,
	...
	return <div className={styles.backgroundMap}></div>;
};
